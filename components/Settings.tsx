...
export const Settings: React.FC<Props> = ({ config, onSave, onLogout }) => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-surface rounded-lg shadow-md border border-border">
      <h2 className="text-2xl font-bold mb-6 text-text-main">Settings & Configuration</h2>
      
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block text-sm font-medium mb-2 text-text-main">Theme</label>
          <select
        ...
        <div>
          <label className="block text-sm font-medium mb-2 text-text-main">Google Apps Script Web App URL</label>
          <input
            type="url"
            required
          ...
          />
          <p className="text-xs text-text-muted mt-1">From your Google Sheet -> Extensions -> Apps Script -> Deploy -> Web App</p>
        </div>

        <div>
          <label className="block text-sm font-medium mb-2 text-text-main">Allowed Emails (Comma separated)</label>
          <textarea
            required
          ...
          />
          <p className="text-xs text-text-muted mt-1">Only these emails will be allowed to log in.</p>
        </div>

        <div>
          <label className="block text-sm font-medium mb-2 text-text-main">Base Currency</label>
          <input
            type="text"
            required
            className="w-full p-2 border border-border rounded bg-background text-text-main"
