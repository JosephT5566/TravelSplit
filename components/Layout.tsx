...
export const Layout: React.FC<LayoutProps> = ({ user, isDemo, apiState, onAddClick }) => {
  ...
  return (
    <>
      ...
      <div className="p-4 bg-surface shadow sticky top-0 z-10 flex justify-between items-center transition-colors border-b border-border">
        ...
        <div className="text-xs text-text-muted flex flex-col items-end">
          ...
          <span>
            {isDemo 
              ? 'Demo Mode' 
              : (apiState.lastUpdated ? `Updated ${new Date(apiState.lastUpdated).toLocaleTimeString()}` : '')}
          </span>
        </div>
      </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-surface border-t border-border flex justify-around p-3 z-30 transition-colors pb-safe">
        <NavLink to="/" className={({isActive}) => `flex flex-col items-center transition-colors ${isActive ? 'text-primary' : 'text-text-muted hover:text-text-main'}`}>
          <PieChart size={24} />
          <span className="text-xs mt-1">Stats</span>
        </NavLink>
        <NavLink to="/list" className={({isActive}) => `flex flex-col items-center transition-colors ${isActive ? 'text-primary' : 'text-text-muted hover:text-text-main'}`}>
          <List size={24} />
          <span className="text-xs mt-1">Expenses</span>
        </NavLink>
        <NavLink to="/settings" className={({isActive}) => `flex flex-col items-center transition-colors ${isActive ? 'text-primary' : 'text-text-muted hover:text-text-main'}`}>
