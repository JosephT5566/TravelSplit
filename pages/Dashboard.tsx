...
export const Dashboard: React.FC<DashboardProps> = ({ expenses, apiState, onRefresh, baseCurrency }) => {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className={`p-2 rounded-full bg-primary/10 text-primary hover:bg-primary/20 transition-colors ${apiState.isLoading ? 'animate-spin' : ''}`}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="p-4 bg-surface rounded-xl shadow border-l-4 border-red-500 transition-colors">
          ...
          <p className="text-xl font-bold text-red-500 truncate" title={`${baseCurrency} ${totalExpense.toFixed(0)}`}>
        ...
        <div className="p-4 bg-surface rounded-xl shadow border-l-4 border-green-500 transition-colors">
          ...
          <p className="text-xl font-bold text-green-500 truncate" title={`${baseCurrency} ${totalIncome.toFixed(0)}`}>
      ...
      <div className="bg-surface p-4 rounded-xl shadow transition-colors border border-border">
        <h3 className="font-semibold mb-4 text-text-main">Category Breakdown</h3>
