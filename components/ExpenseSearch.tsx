...
export const ExpenseSearch: React.FC<Props> = ({ expenses, onEdit, onDelete }) => {
  ...
  return (
    <div className="pb-20">
      ...
      <div className="p-4 bg-surface shadow sticky top-0 z-10 flex items-center gap-3 border-b border-border">
        <button onClick={() => navigate(-1)} className="text-text-muted hover:text-text-main">
          <ArrowLeft size={24} />
        </button>
        ...
        {groupedResults.map(group => (
          <div key={group.date}>
            <h3 className="text-sm font-bold text-text-muted mb-2 sticky top-20 bg-background py-1 z-0">
              {format(parseLocalYMD(group.date), 'EEE, MMM d, yyyy')}
            </h3>
            <div className="space-y-3">
              {group.items.map(exp => (
                <div key={exp.id} className="bg-surface p-4 rounded-xl shadow flex justify-between items-center border border-border">
                  <div className="flex-1">
                    <h4 className="font-semibold text-text-main">{exp.item}</h4>
                    <p className="text-sm text-text-muted">{exp.category} • {exp.payer.split('@')[0]}</p>
                  </div>
                  <div className="text-right">
                    <p className={`font-bold ${exp.type === TransactionType.EXPENSE ? 'text-red-500' : 'text-green-500'}`}>
                      {exp.type === TransactionType.EXPENSE ? '-' : '+'}{exp.amount} <span className="text-xs text-text-muted">{exp.currency}</span>
                    </p>
                    <div className="flex gap-3 justify-end mt-1">
                      <button onClick={() => onEdit(exp)} className="text-text-muted hover:text-primary"><Edit2 size={16} /></button>
                      <button onClick={() => onDelete(exp.id)} className="text-text-muted hover:text-red-500"><Trash2 size={16} /></button>
                    </div>
                  </div>
                </div>
              ))}
