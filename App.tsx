      ...
      <div className="min-h-screen bg-background text-text-main font-sans transition-colors duration-300">
        <Routes>
          ...
          <Route path="/login" element={
             user ? <Navigate to="/" /> : <Login onLogin={handleLogin} config={config} demoUser={DEMO_USER} />
          } />
          
          <Route path="/settings" element={
            <SettingsPage config={config} onSave={handleSaveConfig} onLogout={handleLogout} />
          } />
          ...
          <Route path="/" element={
            !user ? <Navigate to="/login" /> : (
              <Layout 
                ...
                isDemo={isDemo} 
                apiState={apiState} 
                onAddClick={() => { setEditingExpense(null); setShowForm(true); }}
              />
            )
          }>
            <Route index element={
