...
export const api = {
  ...
  async fetchExpenses(gasUrl: string, userEmail: string): Promise<Expense[]> {
    ...
    const url = `${gasUrl}?action=read&user=${encodeURIComponent(userEmail)}`;
    ...
    return (json.data || []).map((row: any) => ({
      ...
      item: row.item,
      type: row.amount >= 0 ? TransactionType.INCOME : TransactionType.EXPENSE, // Logic based on amount sign from Sheet
   ...
   */
  async syncTransaction(gasUrl: string, userEmail: string, action: 'add' | 'edit' | 'delete', expense: Expense): Promise<void> {
    ...
    const signedAmount = expense.type === TransactionType.EXPENSE ? -Math.abs(expense.amount) : Math.abs(expense.amount);
