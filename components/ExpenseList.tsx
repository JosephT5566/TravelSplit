...
import { ChevronLeft, ChevronRight, Search, Calendar, Edit2, Trash2 } from 'lucide-react';
...
export const ExpenseList: React.FC<Props> = ({ 
  ...
  const dailyTotal = useMemo(() => {
    return dailyExpenses.reduce((acc, curr) => {
      ...
      return curr.type === TransactionType.EXPENSE ? acc - amount : acc + amount;
    }, 0);
  }, [dailyExpenses]);
    ...
    <div className="pb-20">
      ...
      <div className="bg-surface shadow-sm mb-4 rounded-xl overflow-hidden border border-border">
          ...
          <div className="flex flex-col items-center relative">
            <label className="text-lg font-bold flex items-center gap-2 cursor-pointer hover:text-primary transition-colors text-text-main">
              ...
              <input 
                ...
                value={dateStr}
                onChange={(e) => {
                  if(e.target.valueAsDate) onDateChange(e.target.valueAsDate);
                }}
              />
            </label>
        ...
        <div className="px-4 py-2 bg-background border-t border-border flex justify-between items-center text-sm">
          ...
          <span className={`font-bold ${dailyTotal < 0 ? 'text-red-500' : 'text-green-500'}`}>
            {dailyTotal < 0 ? '-' : ''}{baseCurrency} {Math.abs(dailyTotal).toFixed(1)}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {dailyExpenses.length === 0 ? (
          ...
          dailyExpenses.map(exp => (
            <div key={exp.id} className="bg-surface p-4 rounded-xl shadow flex justify-between items-center border border-border hover:border-primary transition-all">
              <div className="flex-1">
                <div className="flex items-center justify-between pr-4">
                   <h4 className="font-semibold text-text-main">{exp.item}</h4>
                </div>
                ...
                {exp.remark && <p className="text-xs text-text-muted mt-1 italic">"{exp.remark}"</p>}
              </div>
              
              <div className="text-right pl-2">
                <p className={`text-lg font-bold ${exp.type === TransactionType.EXPENSE ? 'text-red-500' : 'text-green-500'}`}>
                  {exp.type === TransactionType.EXPENSE ? '-' : '+'}{exp.amount}
                </p>
                <p className="text-xs text-text-muted font-medium">{exp.currency}</p>
                
                <div className="flex gap-3 justify-end mt-2 opacity-50 hover:opacity-100 transition-opacity">
                  <button onClick={(e) => { e.stopPropagation(); onEdit(exp); }} className="text-text-muted hover:text-primary"><Edit2 size={16} /></button>
                  <button onClick={(e) => { e.stopPropagation(); onDelete(exp.id); }} className="text-text-muted hover:text-red-500"><Trash2 size={16} /></button>
                </div>
              </div>
            </div>
          ))
